...
# make simple model 
model_lm <- lm(NDVI~.-crop_type_natural-ID-dates-month+season-OC-N+log(N)-EC+log(EC)-soil_water_all-soil_temp_all-air_temp_all-prev_air_temp  , data = result)
#crPlots(model_lm)
#plot(model_lm)
summary(model_lm)
# view model status
...
# expand the model
model_lm=update(model_lm,~(.)^2,data=result)
summary(model_lm)

model_lm2=update(model_lm,~.-climate:log(N)-crop_type:log(N)-prev_soil_water:log(EC)-CaCl2:prev_soil_water-soil_type:log(EC)-soil_type:log(N)-CaCl2:log(N)-log(N):log(EC)-climate:season-K:crop_type-P:log(N)-K:log(N)-prev_soil_temp:climate-season:log(EC)-P:crop_type-prev_soil_water:crop_type-CaCl2:climate-prev_soil_water:climate-P:season-P:K-K:season -CaCl2:season-prev_soil_temp:log(EC)-K:log(EC)-CaCl2:P-prev_soil_water:log(N)-K:soil_type-CaCl2:log(EC)-prev_soil_water:soil_type-K:prev_soil_water-prev_soil_water:season-K:prev_soil_temp-CaCl2:K-prev_soil_temp:soil_type-P:prev_soil_water-season:log(N)-P:soil_type-CaCl2:soil_type-P:log(EC)-prev_soil_temp:log(N),data=result)
...
model_lm2=randomForest(NDVI~.-crop_type_natural-dates-ID+season-month,data=result,ntree=800,nodesize=15,mtry=9,importance=TRUE)
...
model_lm2 # find RMSE by taking sqrt of MSE in this output

#(model_lm2$importance[,1]-model_lm2$importanceSD)/model_lm2$importanceSD
...
# hichical clustering based on counts - chooice either wheat (second line) or 3 crops (first line)
cluster_relation_count_Hclust <- read_csv("~/Desktop/soil data analysis/count_hierchical_result_cut1_3crops_grain.csv") # 3 crops
cluster_relation_count_Hclust <- read_csv("~/Desktop/soil data analysis/bio_data/count_hierchical_result_cut1_wheat_grain3.csv") # wheat only
...
clust_num="3" # change cluster number to 1, 2 and 3 when doing this (store in idx_total1, idx_total2 and idx_total3)
...
ggplot(data = bio_subset_hc_count, aes(x = factor(cluster), y = NDVI_cor, fill = factor(cluster))) +
  geom_boxplot() +
...
res=lm(NDVI_cor~as.factor(cluster),data=bio_subset_hc_count) #[-idx_out,], insert when investigating wheat
anova(res)

# use tukey when having more than 2 groups (wheat)
plot(TukeyHSD(aov(NDVI_cor~as.factor(cluster),data=bio_subset_hc_count[-idx_out,])))
TukeyHSD(aov(NDVI_cor~as.factor(cluster),data=bio_subset_hc_count[-idx_out,]))
...
for (i in 1:length(data_matrix)){
  Boot = boot(data_matrix[,i], Function, R=5000)
...
library(cowplot)
plot_grid(p1_CI, p2_CI,p3_CI, ncol=1,rel_heights = c(5,5),
          rel_widths = c(5,5))
